...
class RectangularRoom:
    def __init__(self, x: int, y: int, width: int,
                 height: int):  # __init__ Функция принимает координаты x и y верхнего левого угла и вычисляет нижний правый угол
    ...
    def center(self) -> Tuple[
        int, int]:  # center это “свойство”, которое по сути действует как переменная, доступная только для чтения для
        center_x = int((
                                   self.x1 + self.x2) / 2)  # нашего RectangularRoom класса. В нем описываются координаты “x“ и ”y" центра комнаты.
    ...
    def inner(self) -> Tuple[
        slice, slice]:  # inner Свойство возвращает два “среза”, которые представляют внутреннюю часть нашей комнаты.
        return slice(self.x1 + 1, self.x2), slice(self.y1 + 1,
                                                  self.y2)  # Это часть, которую мы будем “выкапывать” для нашей комнаты в нашем генераторе подземелий.
...
def place_entities(
        room: RectangularRoom, dungeon: GameMap, maximmum_monsters: int, maximum_items: int,
) -> None:
    ...
    for i in range(number_of_monsters):
        ...
        if not any(entity.x == x and entity.y == y for entity in
                   dungeon.entities):  # A мы проверяем рандомные координаты чтоб не стакнулись враги
...
def tunnel_between(  # Функция принимает два аргумента, оба кортежа, состоящие из двух целых чисел.
        ...
        # Он должен возвращать итератор кортежа из двух целых чисел. Все кортежи будут иметь координаты “x“ и ”y" на карте.
...
def generate_dungeon(
    ...
    dungeon = GameMap(engine, MAP_WIDTH, MAP_HEIGHT,
                      entities=[player])  # A entities=[player] добавили чтоб сам игрок был виден в фове
    ...
    for r in range(max_rooms):
        ...
        if any(new_room.intersects(other_room) for other_room in
               rooms):  # Если наша комната пересекается с другой комнатой, то мы используем continue,
            continue  # чтобы пропустить остальную часть цикла.

        dungeon.tiles[
            new_room.inner] = tile_types.floor  # Здесь мы “выкапываем” комнату. То есть, присваиваем нашей комнате параметры floor
